 ...
 * TODO
 */
public class TestIndianaDataOnDevelopmentMachine {
	private static final Log log = LogFactory
			.getLog(TestIndianaDataOnDevelopmentMachine.class);
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println("Limiting to 500.");
		int done = 0;
		System.out.println("\n---------------------------\n\n");
		for (String uri : uris) {
			if (500 <= done++) {
	...
	private static void createDataService() {
		lds = new ExpandingLinkedDataService(
				new RelocatingHttpLinkedDataService(
						...
						"http://localhost:6080/vivo"), new UrisToExpand(
	...
	private static void createDocumentMaker() {
		dm = new StandardVivoDocumentMaker("Indiana data",
				"http://vivo.iu.edu/");
	}
	...
	private static Iterable<String> discover() throws DiscoveryWorkerException {
		...
		Iterable<String> uris = worker
				.getUrisForSite("http://localhost:6080/vivo");
		for (String uri : uris) {
